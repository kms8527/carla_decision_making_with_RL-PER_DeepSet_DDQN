...
class ReplayBuffer:
    ...
    def PER_make_minibatch(self,batch_size):
        ...
        s0, x0_static, a, r, with_final_s1, with_final_x1_static , done = self.get_priority_experience_batch(batch_size)
        ...
        if len(tmp) != 0:
            non_final_s1 = torch.cat([s for s in with_final_s1 if s is not None])
            non_final_x1_static = torch.cat([x for x in with_final_x1_static if x is not None])
        else:
            non_final_s1 = None
            non_final_x1_static = None
...
class decision_driving_Agent:
    ...
    def update_td_error_memory(self,epoch,alpha = 0.7):
        ...
        action_batch = torch.tensor(batch.action).type(torch.LongTensor).cuda()
    ...
    def ddqn_learning(self):  # sample에서 뽑고 backpropagation 하는 과정
        ...
        if non_final_s1 is not None:
            ...
            a_m[non_final_mask] = self.model(non_final_s1, non_final_x1_static).max(1)[1].cuda()
            non_final_a_m = a_m[non_final_mask]
            strait_action_mask = [False if ego_lane % 1 == 0 else index for index, ego_lane in
                                  enumerate(non_final_x1_static[0::3])]
            non_final_a_m = [non_final_a_m[k].item() if k != False else 1 for k in strait_action_mask]
